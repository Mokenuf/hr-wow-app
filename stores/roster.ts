  ...
  actions: {
    async fetchRoster() {
      ...
      try {
        ...
        const { members } = await blizzApi.query('/data/wow/guild/ragnaros/hair-recovery/roster?namespace=profile-us&locale=en_US')
        this.roster = members.filter(m => m.character && m.character.level === 70 && ranksOfInterest.includes(m.rank)).map(m => ({
