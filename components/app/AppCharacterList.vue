<template>
  <BaseTable :columns="columns" :rows="paginatedCharacters">
    <template v-slot:custom-cell="{ row, column }">
      <span
        v-if="column.key === 'class'"
        :style="{ color: getClassColor(row.classId) }"
        >{{ row.class }}</span
      >
...
const characters = computed(() =>
  rosterStore.roster.map((r) => ({
    ...
    level: r.level,
    race:
      appStore.playableRaces.find((pr) => pr.id === r.raceId)?.name ??
      "Unknown",
    class:
      appStore.playableClasses.find((pc) => pc.id === r.classId)?.name ??
      "Unknown",
    classId: r.classId,
  }))
);
